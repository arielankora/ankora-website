...
export function SegmentPage({
  ...
  const link =
    "text-paper underline decoration-gold/40 underline-offset-4 transition-colors hover:text-gold";
  ...
  return (
    <>
      ...
      <SectionShell>
        <Reveal>
          <p className="max-w-[72ch] font-assistant font-light leading-[1.8] text-tone-muted">
            {bridge.pre}
          ...
          <HairlineGrid minCell={280}>
            {content.bullets.map((bullet) => (
              <motion.div key={bullet.title} variants={staggerItem} className="h-full">
                <HairlineGridCell>
                  <h2 className="text-[1.12rem] font-normal text-paper">{bullet.title}</h2>
                ...
                <motion.div key={item.href} variants={staggerItem} className="h-full">
                  <Link href={withLocale(locale, item.href)} className="group block h-full">
                    <HairlineGridCell className="transition-colors duration-[350ms] group-hover:bg-[rgba(176,141,87,0.08)]">
                      <h2 className="text-base font-medium text-paper transition-colors group-hover:text-gold">
                        {item.label}
                      </h2>
                      <p className="mt-2 font-assistant text-sm font-light leading-[1.7] text-tone-muted">
                        {item.blurb}
