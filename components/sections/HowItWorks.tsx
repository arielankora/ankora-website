...
function LaneHeader({ label, attention, gold }: { label: string; attention: string; gold?: boolean }) {
  return (
    <div className="mb-1 flex items-baseline justify-between gap-3">
      <span
        className={`font-assistant text-[13.5px] font-semibold ${gold ? "text-gold" : "text-cream"}`}
      >
        {label}
      </span>
      <MonoLabel className="text-muted">{attention}</MonoLabel>
    </div>
  );
}
...
function SwimLanes({ dict }: { dict: Dictionary }) {
  ...
  return (
    ...
    <div className="bg-[rgba(243,234,219,0.04)] p-[clamp(24px,3.4vw,44px)] outline outline-1 outline-[rgba(243,234,219,0.11)] backdrop-blur-[16px]">
      <MonoLabel size={10} tracking="0.16em" className="text-muted">
        {h.railLabel}
      </MonoLabel>
      ...
      >
        <div>
          ...
          >
            {TICKS.map((tick) => (
              ...
              >
                <span
                  className="absolute -top-[3px] h-1.5 w-1.5 rounded-full bg-cream"
          ...
          <div className="relative mt-[29px] h-[3px] bg-[rgba(243,234,219,0.14)]" aria-hidden="true">
            <span className="absolute top-1/2 -mt-1.5 h-3 w-3 animate-tokenSlow rounded-full bg-cream" />
            <span
              className="absolute -top-1.5 h-[15px] w-0.5 bg-[rgba(243,234,219,0.4)]"
              style={{ insetInlineEnd: 0 }}
            />
          </div>
          <p className="mt-[18px] max-w-[50ch] font-assistant text-sm font-light leading-[1.75] text-muted">
            {h.noteWithout}
          </p>
        </div>
        ...
        <div>
          ...
          <p className="mt-[18px] max-w-[50ch] font-assistant text-sm font-light leading-[1.75] text-body">
            {h.noteWith}
          </p>
        </div>
      </div>
...
export function HowItWorks({ dict }: { dict: Dictionary }) {
  return (
    <SectionShell>
      ...
      <Reveal delay={0.14}>
        <p className="mt-4 max-w-[40ch] font-assistant font-light text-muted">
          {dict.howItWorks.sub}
        </p>
      </Reveal>
        ...
        <RevealStagger className="grid gap-px [grid-template-columns:minmax(0,1fr)] min-[641px]:[grid-template-columns:repeat(2,minmax(0,1fr))] min-[1181px]:[grid-template-columns:repeat(5,minmax(0,1fr))]">
          {dict.howItWorks.steps.map((step, i) => (
            ...
            >
              ...
              <h3 className="mt-3.5 text-[1.12rem] font-normal leading-[1.3] text-cream">
                {step.title}
              </h3>
              <p className="mt-2.5 font-assistant text-sm font-light leading-[1.7] text-muted">
                {step.body}
              </p>
            </motion.div>
          ))}
