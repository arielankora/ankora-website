...
export function EditCategoryForm({ category }: { category: Category }) {
  ...
  return (
    <form action={formAction} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
      ...
      <div>
        <label className="block text-xs font-medium text-appNavy/60">תיאור</label>
        <input
          name="description"
      ...
      <div>
        <label className="block text-xs font-medium text-appNavy/60">סדר הצגה</label>
        <input
          name="sortOrder"
          type="number"
      ...
      <div className="flex items-center gap-2 self-end pb-2">
        ...
        />
        <label htmlFor="active" className="text-sm text-appNavy/70">
          פעילה
        </label>
      </div>
