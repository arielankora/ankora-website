  ...
  return (
    <>
      <div className="space-y-6">
        <div>
          <Link href="/app/important-dates" className="text-xs text-appNavy/50 hover:text-gold-dim">
            ← חזרה למועדים חשובים
          </Link>
          <div className="mt-2 flex flex-wrap items-center justify-between gap-4">
            <div>
              <h1 className="text-xl font-medium text-appNavy">{date.title}</h1>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          <div className="space-y-6 lg:col-span-2">
            <div className="rounded-2xl border border-lineDark bg-white p-6">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-medium text-appNavy">פרטים</h2>
                <StatusBadge label={IMPORTANT_DATE_STATUS_LABELS[date.status]} tone={STATUS_TONE[date.status]} />
              </div>
              <dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                <div>
                  <dt className="text-xs text-appNavy/50">מופע הבא</dt>
                  <dd className="text-appNavy">{formatDate(date.nextOccurrenceAt)}</dd>
                </div>
                <div>
                  <dt className="text-xs text-appNavy/50">אחראי</dt>
                  <dd className="text-appNavy">{date.responsibleUser.name}</dd>
                </div>
                <div>
                  <dt className="text-xs text-appNavy/50">לוח שנה</dt>
                  <dd className="text-appNavy">{date.calendarType === "HEBREW" ? "עברי" : "לועזי"}</dd>
                </div>
                <div>
                  <dt className="text-xs text-appNavy/50">רגישות</dt>
                  <dd className="text-appNavy">{date.sensitivity === "SENSITIVE" ? "רגיש" : "רגיל"}</dd>
                </div>
                {date.notes && (
                  <div className="col-span-2">
                    <dt className="text-xs text-appNavy/50">הערות</dt>
                    <dd className="text-appNavy">{date.notes}</dd>
                  </div>
                )}
              </dl>
            </div>
            ...
            <div className="rounded-2xl border border-lineDark bg-white p-6">
              <h2 className="text-sm font-medium text-appNavy">תזכורות</h2>
              {date.reminderRules.length === 0 ? (
                <p className="mt-2 text-sm text-appNavy/50">אין תזכורות מוגדרות למועד זה.</p>
              ) : (
                <ul className="mt-3 space-y-2">
                  {date.reminderRules.map((rule) => (
                    <li key={rule.id} className="flex items-center gap-3 text-sm text-appNavy/70">
                      <span className="rounded-full bg-appNavy/5 px-3 py-1 text-xs text-appNavy">{rule.daysBefore} ימים לפני</span>
                      {rule.sendInApp && <span className="text-xs text-appNavy/50">בתוך המערכת</span>}
                      {rule.sendEmail && <span className="text-xs text-appNavy/50">מייל</span>}
            ...
            {date.tasks.length > 0 && (
              <div className="rounded-2xl border border-lineDark bg-white p-6">
                <h2 className="text-sm font-medium text-appNavy">משימות שנוצרו אוטומטית</h2>
                <ul className="mt-3 space-y-1.5">
                  {date.tasks.map((task) => (
                    <li key={task.id} className="text-sm text-appNavy/70">
                      {task.title}
                    </li>
                  ))}
                </ul>
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border border-lineDark bg-white p-6">
              <h2 className="text-sm font-medium text-appNavy">סטטוס</h2>
              <div className="mt-3">
            ...
            <div className="rounded-2xl border border-lineDark bg-white p-6">
              <h2 className="text-sm font-medium text-appNavy">דחיית טיפול (Snooze)</h2>
              <p className="mt-1 text-xs text-appNavy/50">דוחה את הטיפול מבלי לשנות את תאריך האירוע המקורי.</p>
              <form action={snoozeImportantDateAction} className="mt-3 flex items-center gap-2">
                ...
                <input
                  ...
                  name="snoozedUntil"
                  required
                  className="w-full rounded-lg border border-lineDark bg-white px-3 py-2 text-sm text-appNavy outline-none focus:border-gold"
                />
                <button type="submit" className="whitespace-nowrap rounded-full border border-lineDark px-4 py-2 text-sm text-appNavy hover:border-gold">
                  דחייה
                </button>
              </form>
            </div>
            ...
            <div className="rounded-2xl border border-lineDark bg-white p-6">
              <h2 className="text-sm font-medium text-appNavy">מחיקה</h2>
              <p className="mt-1 text-xs text-appNavy/50">מעביר לארכיון רך - ניתן לשחזר דרך בסיס הנתונים אם צריך.</p>
