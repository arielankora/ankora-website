...
export default defineConfig({
  ...
  timeout: 30_000,
  // Back to 10s, which is where it belongs.
  //
  // It was raised to 20s as openly-declared compensation for a product
  // fault: writes were taking tens of seconds, seven specs in five files
  // went flaky on it in one run, and padding each one separately was
  // burying the cause in whichever spec happened to notice it.
  //
  // Both causes have since been found and fixed - the dashboard's
  // per-client hour-bank queries (#86) and alert evaluation running
  // inside every write (#87), with flakiness dropping from six specs to
  // one. So the compensation is now costing more than it gives: a
  // 20-second default does not only absorb the fault that is gone, it
  // also absorbs the next screen that becomes slow for its own reasons,
  // and absorbs it silently.
  //
  // Worth keeping as a rule: a timeout raised for a known reason has to
  // come back down when that reason does, or it stops being a
  // measurement and becomes a blindfold.
